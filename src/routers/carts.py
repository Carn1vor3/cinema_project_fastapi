...
async def add_movie_to_cart(movie_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    existing_item = await db.scalar(select(CartItems).where(CartItems.cart_id == cart.id, CartItems.movie_id == movie_id))
    if existing_item:
...
async def remove_movie_from_cart(movie_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    cart = await db.scalar(select(Carts).where(Carts.user_id == current_user.id))
    if not cart:
        raise HTTPException(status_code=404, detail="Cart not found.")

    item = await db.scalar(select(CartItems).where(CartItems.cart_id == cart.id, CartItems.movie_id == movie_id))
    if not item:
...
@router.get("/view")
async def view_cart(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    cart = await db.scalar(select(Carts).where(Carts.user_id == current_user.id).options(selectinload(Carts.items).selectinload(CartItems.movie).selectinload(Movies.genres)))
...
async def view_user_cart_for_admin(user_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.group.name not in [UserGroupEnum.ADMIN, UserGroupEnum.MODERATOR]:
        raise HTTPException(status_code=403, detail="Not authorized.")

    cart = await db.scalar(select(Carts).where(Carts.user_id == user_id).options(selectinload(Carts.items).selectinload(CartItems.movie).selectinload(Movies.genres)))
    if not cart or not cart.items:
        return []
